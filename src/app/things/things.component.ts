...
export class ThingsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.thingsService.getThings(data)
      .subscribe(
        (response) => {
  ...
  addThing(form : NgForm) {
    ...
    this.thingsService.addThing(body).subscribe(
      (response) =>{
        const data = {
          page_no: ''
        };
  ...
  onFileChange(file: FileList) {
    ...
    addCsvFilefn(form : NgForm){
      ...
      this.thingsService.addCsvThings(data).subscribe(
        (response) =>{
          if (response.success === 1) {
            this.isLoading = false;
            const data = {
    ...
    deleteWord(id) {
      ...
      if (this.NoInternet === false) {
        ...
        this.thingsService.deleteWord(id).subscribe(
          (response) =>{
            if (response.success === 1) {
              this.isLoading = false;
              const data = {
