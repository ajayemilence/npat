...
export class PlantsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.plantsService.getPlants(data)
      .subscribe(
        (response) => {
          if (response.success === 1) {
            ...
            this.totalNumPages = response.data.numPages;
  ...
  addPlant(form : NgForm) {
    ...
    this.plantsService.addPlant(body).subscribe(
      (response) =>{
        console.log(response);
        const data = {
          page_no: ''
  ...
  getwords(data) {
    console.log("get words hai" , data);    
    this.plantsService.getPlants(data)
      .subscribe(
        (response) => {
          if (response.success === 1) {
  ...
  onFileChange(file: FileList) {
    ...
    addCsvFilefn(form : NgForm){
      ...
      this.plantsService.addCsvPlace(data).subscribe(
        (response) =>{
          if (response.success === 1) {
            this.isLoading = false;
            const data = {
    ...
    deleteWord(id) {
      ...
      if (this.NoInternet === false) {
        ...
        this.plantsService.deleteWord(id).subscribe(
          (response) =>{
            if (response.success === 1) {
              this.isLoading = false;
              const data = {
      ...
      console.log(this.currentPage);
        if (this.currentPage - 1 === this.totalPages[4]) {
          ...
          if (this.currentPage < this.totalNumPages ) {
            ...
            this.isLoading = true;
            console.log("else part ka data" , data);
            this.getwords(data);
          }
        }
   }
