...
export class AnimalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.animalsService.getAllAnimals(data)
      .subscribe(
        (response) => {
  ...
  addAnimal(form : NgForm) {
    ...
    this.animalsService.addAnimal(body).subscribe(
      (response) =>{
        console.log(response);
        const data = {
          page_no: ''
...
onFileChange(file: FileList) {
  ...
  addCsvFilefn(form : NgForm){
    ...
    this.animalsService.addCsvAnimals(data).subscribe(
      (response) =>{
        if (response.success === 1) {
          this.isLoading = false;
          const data = {
  ...
  deleteWord(id) {
    ...
    if (this.NoInternet === false) {
      ...
      this.animalsService.deleteWord(id).subscribe(
        (response) =>{
          if (response.success === 1) {
            this.isLoading = false;
            const data = {
